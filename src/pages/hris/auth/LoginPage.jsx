...
import { FEATURE_ROUTE_MAP } from "@/utils/featureRedirectMap";
  ...
  const handleLogin = async (e) => {
    ...
    if (!email || !password) {
      glassToast({
        ...
        icon: <ExclamationTriangleIcon className="text-red-700 w-5 h-5" />,
        textColor: "white",
    ...
    try {
      ...
      glassToast({
        message:
          ...
          "Login failed", // idk where this "err?.response?.data?.message" coming from
        icon: <ExclamationTriangleIcon className="text-red-600 w-5 h-5" />,
        textColor: "white",
        bgColor: "rgba(255, 255, 255, 0.2)",
        blur: 12,
        duration: 4000,
    ...
    <div className="flex p-10 items-center justify-center h-screen bg-gradient-to-tl from-[#008080] to-[#CC5500]">
      <div className="w-full max-w-md space-y-6 p-8 rounded-2xl shadow-xl border border-white/20 bg-white/10 backdrop-blur-md">
        ...
        <div className="text-center ">
          <p className="text-2xl font-semibold  text-white">HRIS + ATS</p>
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          ...
          <Button
            type="submit"
            className="w-full text-primary-color border-none hover:bg-gray-100"
          >
            Log in
          </Button>
        </form>
