...
const { personalInfo, user } = useContext(UserContext);
...
//   const user = true;
 ...
 const filteredSections = sidebarLinks.filter((section) => {
  const isServiceAllowed = allowedServices.includes(section.service.toLowerCase());
    ...
    <>
      ...
      >
        ...
        >
          <div
            className={`${
              collapsed ? "h-11 w-11 mt-1.5" : "h-11 w-11"
          ...
          {!collapsed && personalInfo && (
            <div className="overflow-hidden ml-3">
              {/* <p className="text-[#008080] truncate text-xs">SUPERADMIN</p> */}

              <h3 className="font-medium text-gray-900 truncate text-medium">
                {personalInfo.first_name} {personalInfo.last_name}
              </h3>
              <p className="text-gray-400 truncate text-xs">{user.user_email}</p>
            </div>
          )}
        </div>
